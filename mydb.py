...
class Database:
    def __init__(self, db):
        ...
        self.cur.execute(
            "CREATE TABLE IF NOT EXISTS expense_record (item_name varchar2, item_price float, purchase_date date)")
        self.conn.commit()

    def fetchRecord(self, query):
        self.cur.execute(query)
        #This code fetches all the rows from the current cursor position in the database and stores them in the variable "rows".
